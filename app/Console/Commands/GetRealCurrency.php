...
//        $currencies = ['EUR', 'USD', 'CAD'];
        ...
        foreach (ExchangeRates::CURRENCY_LIST as $currency)
        {
            $todayCurrency = ExchangeRates::getCurrency($currency);
            if($todayCurrency != null)
            {
                continue;
            }

            $response = Http::get("https://kurs.resenje.org/api/v1/currencies/$currency/rates/today"); // Domaci API bez autentifikacije test
